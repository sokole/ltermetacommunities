...
fn.mc.loop <- function(comm.wide = comm.wide, output = output){
  ...
  for(year.i in unique(comm.wide$year)){
    ...
    # Write Output
    site.index <- which(output$year == year.i)
    output[which(output$year == site.index),] <- c(site.index,
...
mc.time.series.summary <- fn.mc.loop(comm.wide = nwt.comm.wide, out = nwt.time.series)
```
