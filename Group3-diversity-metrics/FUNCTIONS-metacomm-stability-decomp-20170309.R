...
#####################################
#####################################
# Below are functions from Legendre
# necessary for the above calculations
#####################################
#####################################
#####################################
#####################################
...
# Jaccard or Sørensen dissimilarity into replacement and richness difference 
# components, for species presence-absence or abundance data.

beta.div.comp <- function(mat, coef="J", quant=FALSE, save.abc=FALSE)
  #
  # Description --
  # 
  # Compute the Podani- or Baselga-family decomposition of the Jaccard or Sørensen 
  # dissimilarity coefficient into replacement and richness difference components, 
  # for species presence-absence or abundance data, as described in Legendre 
  # (submitted).
  #
  # Usage --
  #
...
# rich : Richness difference matrix, class = 'dist'.
...
#        3. Total richness difference diversity
#        4. Total replacement div./Total beta div.
#        5. Total richness difference div./Total beta div.
...
# 825–834.
#
# Legendre, P. Replacement and richness difference components of beta diversity: 
# some developments. Submitted manuscript.
#
...
# Author:: Pierre Legendre
{
  ...
  if(!quant) {      # Binary data provided, or make the data binary
    if(coef==1) form="Podani family, Sørensen" 
    if(coef==2) form="Podani family, Jaccard"
    if(coef==3) form="Baselga family, Sørensen" 
...
LCBD.comp <- function(x, sqrt.x=TRUE)
  #
  # Computes LCBD indices for beta diversity components (Repl. and Rich.)
  #
  # Arguments --
  #
  # x : beta diversity component matrix, class=dist (from beta.div.comp output).
  # sqrt.x : take sqrt() of components before computing LCBD.comp. Use
  #          sqrt.x=TRUE for all indices computed by function beta.div.comp().
  #
  # Reference --
  #
...
# Author:: Pierre Legendre, August 2013
{
  ...
  if(sqrt.x) {
    ...
    delta1 <- centre(as.matrix(-0.5*x), n)
  } else {
    SStotal <- sum(x^2)/n      # eq. 8
    BDtotal <- SStotal/(n-1)   # eq. 3
    delta1 <- centre(as.matrix(-0.5*x^2), n)
  }
  
  LCBD <- diag(delta1)/SStotal   # Legendre & De Caceres (2013), eq. 10b
  #
  out <- list(SStotal_BDtotal=c(SStotal,BDtotal), LCBD=LCBD, D=n)
}