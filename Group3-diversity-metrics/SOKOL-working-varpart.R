...
dat.pcnm <- d.pcnm.wide %>% 
  arrange(SITE_ID) %>%
  na.omit()%>% 
  select(-SITE_ID)

# ---------------------------------------------------------------------------------
# pre-screen for environmental vars with a pval < 0.20
...
# the dbRDA R2 values
R2.ab <- RsquareAdj(mod.ab)$adj.r.squared
R2.bc <- RsquareAdj(mod.bc)$adj.r.squared
R2.abc <- RsquareAdj(mod.abc)$adj.r.squared
...
write.csv(dat.varpart, 'TABLE_dbRDA_haplotype varpart_results.csv',
          row.names = FALSE)
