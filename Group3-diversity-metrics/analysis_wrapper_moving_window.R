...
for(i in 1:nrow(data_list)){
  ...
  try_result <- try({
    ...
    #############################
    # apply ltmc to moving window
    
    # initialize moving window 
    window_size <- 2
    year_list <- d.in.long$DATE %>% unique %>% as.numeric()
    i_start_year <- year_list[1]
    i_end_year <- i_start_year + window_size - 1
    ...
    while(i_end_year <= max(year_list)){
      ...
      d.bd.h_window <- data.frame()
      d.bd.hT_window <- data.frame()
      d.bd.agg_window <- data.frame()
      div.part.var_window <- data.frame()
      
      if(nrow(d.window.long) > 0){
        ...
        start_year <- min(as.numeric(d.window.long$DATE))
        end_year <- max(as.numeric(d.window.long$DATE))
        n_years_observed <- d.window.long$DATE %>% unique() %>% length()
        study_duration <- end_year - start_year
        
        # d.bd.h_window
        d.bd.h_window <- data.frame()
        d.bd.h_window <- ltmc::metacommunity_variability(
          data_long = d.window.long,
        ...
        # d.bd.hT_window
        d.bd.hT_window <- data.frame()
        d.bd.hT_window <- ltmc::metacommunity_variability(
          data_long = d.window.long,
          site_id_col_name = 'SITE_ID',
          time_step_col_name = 'DATE',
          taxon_id_col_name = 'VARIABLE_NAME',
        ...
        # d.bd.agg_window
        d.bd.agg_window <- data.frame()
        d.bd.agg_window <- ltmc::metacommunity_variability(
          data_long = d.window.long,
          site_id_col_name = 'SITE_ID',
          time_step_col_name = 'DATE',
          taxon_id_col_name = 'VARIABLE_NAME',
        ...
        analysis_results_i <- data.frame(
          ...
          organism_count_type = d.window.long$VARIABLE_UNITS %>% unique() %>% paste(collapse = ' | '),
          bind_rows(
            div.part.var_window,
            d.bd.agg_window,
            d.bd.h_window,
...
#                     paste0('moving_window_analysis_results_', Sys.Date(), '.csv')),
#           row.names = FALSE)
