...
I{\bf n prep.
Need to fix cleaning script.
Object r\_ids not found. 
Remove duplicate row identifiers so we can add zeros. 
Maybe once I can run the cleaning script, the sampling effort plot will look better.}
Data are shown in Figure \ref{jrn-plants}.

Intended methods:
We selected a subsample of 6 quadrats that provided 14 years of continuous observations. 
...
Data were downloaded from EDI \citep{bes-birds}. We dropped all observations in the distance catagory "FT" and all observations not identified to species. We only used sites with surveys every year from 2005-2009. When there was multiple surveys per year for a single site, the abundance counts were aggregated to the maximum observed count from any survey for each species at the respective site.
