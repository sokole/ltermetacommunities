...
{\bf Updated data are on EDI and L3 data/cleaning script need to be updated for this dataset.}
Two of the eleven sites were initiated in the third year of study.
I think we should remove the two sites.
...
{\bf Do we need to separate by habitat type? Needs data package citation.}
...
{\bf Do we need to separate by habitat type?}
...
\subsection {ntl-fish}
{\bf Need to decide which lakes/years to include BEFORE including in analysis.}
...
Also, the two bog lake sites (CB, TB) have only one species reported (see shared species plot in Figure \ref{ntl-fish}. 
Do they belong in the analysis? Probably not.
Three of the 11 sites (FI, MO, WI) were initiated 14 years after the others.
There is a jump in the cumulative number of species after those sites began to be sampled.
Those three sites could be removed but I think it would be better to use the shorter time series for all nine lakes.
...
\caption{{\bf NTL-fish:} Species accumulation curves (top left),  annual richness (top right), and sampling effort (bottom)  for 134 fish species observed at 11 sites North Temperate Lakes LTER (1981-2016). The black lines represent total site-level values across all plots.}
\label{ntl-fish}
\end{figure}
...
{\bf Needs work.}
The data were downloaded from the EDI Data Portal (knb-lter-and.4781.2).
These are point count data.
Need to re-think how the `count' is calculated.
Should not be the simple sum of all six rounds per summer; perhaps max would be better.
Also, the same bird could be observed multiple times during the 10-minute observation interval if he sang more than once.
The NEW{\_}RECORD column indicates this status with binary 0/1, and our final count should only include NEW{\_}RECORD = 1.
Note that all six rounds per growing season were only done in the initial five years of study.
Only one or two rounds per growing season were done in 2014-2016, likely causing the dip in richness in those years.
Fix this.
Data not propagated.
