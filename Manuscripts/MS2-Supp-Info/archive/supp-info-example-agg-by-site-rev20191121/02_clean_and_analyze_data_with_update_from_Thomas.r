...
data.long.algae.site.means.by.year <- data.long.algae %>%
  group_by(YEAR, SITE, SP_CODE) %>%
  summarize(SFDM_mean = mean(SFDM))

# identify and remove transects that have years with zero biomass -- analysis won't work
site_by_year_summary <- data.long.algae.site.means.by.year %>% 
  group_by(SITE, YEAR) %>% 
  summarize(TOT_SFDM = sum(SFDM_mean))

site_years_with_0_biomass <- site_by_year_summary %>%
  filter(TOT_SFDM == 0)

data.long.algae.site.means.by.year <- data.long.algae.site.means.by.year %>%
  filter(!SITE %in% site_years_with_0_biomass$SITE)
...
data.long.analysis <- data.long.algae.site.means.by.year %>%
  select(SITE, YEAR, SP_CODE, SFDM_mean) %>%
  rename(DATE = YEAR,
         SITE_ID = SITE,
         VALUE= SFDM_mean)
...
plot.tot.bio.local <- cbind(as.data.frame(data.wide.analysis[,c("SITE_ID", "DATE")]), tot.bio) 
