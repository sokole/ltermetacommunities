...
# 1. IMPORT DATA SETS ------------------------------------------------------------
df <- read_csv("output/metacom_data_for_models_20190311.csv")
...
(phi_vs_gamma_var <- ggplot(aes(x = phi_var, y = gamma_var, label = site),
    ...
    geom_label(size = 2) +
    xlab("Phi variability") +
    ylab(expression(paste("Gamma variability"))) +
    annotate("text", x = 0.2, y = 0.6, label = "italic(P) < 0.001", parse = TRUE) 
)

jpeg(file = "metacommunity_components_h.jpeg", width = 6.75, height = 3.25, units = 'in', res = 1000)
ggarrange(alpha_vs_gamma_var, phi_vs_gamma_var, 
          ncol = 2, nrow = 1)
dev.off()
...
(phi_size_biome <- ggplot(h.wide, aes(x = body.size, y = phi_var, 
   ...
   theme(legend.position = "none") +
    ggsave("body.size_by_biome_phi.jpeg"))
...
(gam_size_biome <- ggplot(h.wide, aes(x = body.size, y = log(gamma_var), 
    ...
    theme_bw()+
    theme(legend.position = "none") +
  ggsave("body.size_by_biome_gamma.jpeg"))
...
(alpha_size_biome <- ggplot(h.wide, aes(x = body.size, y = log(alpha_var), 
    ...
    theme_bw() +
    theme(legend.position = "none") +
    ggsave("body.size_by_biome_alpha.jpeg"))

jpeg(file = "bodysize_biome_h.jpeg", width = 6.5, height = 5, units = 'in', res = 1000)
