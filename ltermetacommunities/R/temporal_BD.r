...
temporal_BD <- function(
  ...
  if(is.null(time_step_col_name) & is.null(taxon_id_col_name) & is.null(biomass_col_name)){
    ...
    n_numeric_cols <- sum(data_in %>% summarize_all(is.numeric) %>% as.numeric())
  ...
  if(!is.null(taxon_list) & length(taxon_list) > 1){
    ...
    comm_long <- data_in %>%
      gather('taxon_id','biomass', one_of(taxon_list)) %>%
      select(one_of(time_step_col_name, taxon_id_col_name, biomass_col_name))
  ...
  comm_long <- comm_long %>% select(one_of(time_step_col_name), taxon_id, biomass)
